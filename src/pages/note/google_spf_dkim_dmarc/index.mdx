...
本記事では基本的に会社で行った内容をベースにどう設定したのかを記述していく。
...
これは Gmail 側で行われる認証ロジックによって、ドメインを取得した本人（もしくは法人）から送信されているメールかどうか判断できなかった場合に表示される。この問題を回避するために Google のサポートでは SPF、DKIM、DMARC の設定を推奨している[^1]。
...
DNS レコードに SPF であればホストや IP などのリストを、DKIM だと検証に必要な公開鍵を記録することで機能する。
...
- DMARC (Domain-based Message Authentication, Reporting, and Conformance)
  ...
  - 送信者が受信者に対して、認証に失敗したメールをどう扱って欲しいか DNS レコードでポリシーを公開することで指示が可能
